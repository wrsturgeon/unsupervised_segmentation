...
from jax import devices, jit, numpy as jnp, vmap
...
# This file explores one way to work around that by separating functions and data
...
def fork_normalize(p: list, aux_std: jnp.ndarray = 1) -> list:
    ...
    return p
def fork(p: list, x: jnp.ndarray, stochastic: bool = False) -> jnp.ndarray:
    out = _fork(p[0], x, stochastic)
    p[1] = update_running_std(p[1], jnp.std(out))
    return out
@partial(jit, static_argnames=["stochastic"])
def _fork(p: list, x: jnp.ndarray, stochastic: bool = False) -> jnp.ndarray:
...
def linear_normalize(p: list, aux_std: jnp.ndarray = 1) -> list:
    ...
    return p
def linear(p: list, x: jnp.ndarray, stochastic: bool = False) -> jnp.ndarray:
    out = _linear(p[0], x, stochastic)
    p[1] = update_running_std(p[1], jnp.log2(jnp.std(out, 1)))
    return out
@partial(jit, static_argnames=["stochastic"])
def _linear(p: jnp.ndarray, x: jnp.ndarray, stochastic: bool = False) -> jnp.ndarray:
...
def feedforward_normalize(p: list, aux_std: jnp.ndarray = 1) -> list:
    ...
    return p
def feedforward(p: list, x: jnp.ndarray, stochastic: bool = False) -> jnp.ndarray:
    out = _feedforward(p[0], x, stochastic)
    p[1] = update_running_std(p[1], jnp.std(out))
    return out
@partial(jit, static_argnames=["stochastic"])
def _feedforward(p: list, x: jnp.ndarray, stochastic: bool = False) -> jnp.ndarray:
...
def mhsa_normalize(p: list, aux_std: jnp.ndarray = 1) -> list:
    ...
    return p
def mhsa(p: list, x: jnp.ndarray, stochastic: bool = False) -> jnp.ndarray:
    out = _mhsa(p[0], x, stochastic)
    p[1] = update_running_std(p[1], jnp.std(out))
    return out
@partial(jit, static_argnames=["stochastic"])
def _mhsa(p: list, x: jnp.ndarray, stochastic: bool = False) -> jnp.ndarray:
...
def mhsa_res_normalize(p: list, aux_std: jnp.ndarray = 1) -> list:
    ...
    return p
def mhsa_res(p: list, x: jnp.ndarray, stochastic: bool = False) -> jnp.ndarray:
    out = _mhsa_res(p[0], x, stochastic)
    p[1] = update_running_std(p[1], jnp.std(out))
    return out
@partial(jit, static_argnames=["stochastic"])
def _mhsa_res(p: list, x: jnp.ndarray, stochastic: bool = False) -> jnp.ndarray:
...
def feedforward_res_normalize(p: list, aux_std: jnp.ndarray = 1) -> list:
    ...
    return p
def feedforward_res(p: list, x: jnp.ndarray, stochastic: bool = False) -> jnp.ndarray:
    out = _feedforward_res(p[0], x, stochastic)
    p[1] = update_running_std(p[1], jnp.std(out))
    return out
@partial(jit, static_argnames=["stochastic"])
def _feedforward_res(p: list, x: jnp.ndarray, stochastic: bool = False) -> jnp.ndarray:
...
def encoder_block_update(p: list, dLdp: list, lr: jnp.float32) -> list:
    p[0][0] = mhsa_res_update(p[0][0], dLdp[0][0], lr)
    p[0][1] = feedforward_res_update(p[0][1], dLdp[0][1], lr)
...
def encoder_block_normalize(p: list, aux_std: jnp.ndarray = 1) -> list:
    ...
    return p
def encoder_block(p: list, x: jnp.ndarray, stochastic: bool = False) -> jnp.ndarray:
    out = _encoder_block(p[0], x, stochastic)
    p[1] = update_running_std(p[1], jnp.std(out))
    return out
@partial(jit, static_argnames=["stochastic"])
def _encoder_block(p: list, x: jnp.ndarray, stochastic: bool = False) -> jnp.ndarray:
...
# Get JAX's error messages out of the way
